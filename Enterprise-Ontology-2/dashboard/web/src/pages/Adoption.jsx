...
const Adoption = () => {
  ...
  const handleExportPPT = async () => {
    setIsExporting(true);
    try {
      const response = await fetch('/api/adoption/export-ppt');
      ...
      a.href = url;
      a.download = `Nexus_Adoption_Report_${new Date().toISOString().split('T')[0]}.pptx`;
        ...
        badge={
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              onMouseEnter={(e) => e.target.style.background = '#252542'}
              onMouseLeave={(e) => e.target.style.background = '#1a1a2e'}
            >
              {isExporting ? (
                <>
                  <div className="animate-spin w-4 h-4 border-2 border-white/50 border-t-transparent rounded-full" />
                  <span style={{ color: '#9ca3af' }}>Exporting...</span>
                </>
              ) : (
                <>
                  <FileDown size={16} color="#22d3ee" />
                  Export PPT
                </>
              )}
            </button>
